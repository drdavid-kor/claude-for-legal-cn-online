...
assert.match(workerSource, /模型服务返回空响应/, "worker explains empty provider responses");
assert.match(workerSource, /非 JSON 响应/, "worker explains non-JSON provider responses");
assert.match(workerSource, /OpenAI-compatible 格式/, "worker explains invalid provider response shape");
